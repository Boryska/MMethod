       ...
       {        if(i != n) {
        ...
        while( ((alfa[k].multiply(1000000000).add(betta[k])).compareTo(new BigFraction(0))) < 0) {
            ...
            if (check == 0) {
                ...
                alert.setContentText("Задача не иммет решения из-за неогнраниченности целевой функции сверху!");
                alert.showAndWait();
                return;
        ...
        for (int i = 0; i < Fs.length ; i++) {
            if(Fs[i] > n && newA[i][0].doubleValue() != 0){
                ...
                alert.setTitle("Информация о решении");
                alert.setHeaderText("\"При решении исходной задачи был получен следующий результат\"");
                alert.setContentText("Задача не имеет решения, так есть несовместимые условия\n!");
                alert.showAndWait();
                return;
            }
        }

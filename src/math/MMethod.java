       ...
       {        if(i != n) {
        ...
        while( ((alfa[k].multiply(1000000000).add(betta[k])).compareTo(new BigFraction(0))) < 0) {
            ...
            if (check == 0) {
                ...
                alert.setContentText("Исходная задача не иммет решения из-за неогнраниченности целевой функции сверху!");
                iterat.append("\n\nИсходная задача не иммет решения из-за неогнраниченности целевой функции сверху!");
                zvit1.append("\n\nИсходная задача не иммет решения из-за неогнраниченности целевой функции сверху!");
        ...
        for (int i = 0; i < Fs.length ; i++) {
            if(Fs[i] > n && newA[i][0].doubleValue() != 0){
                zvit1.append("\n Исходная задача не имеет решения, так есть несовместимые условия!");
                ab.append("\n Исходная задача не имеет решения, так есть несовместимые условия!");
                listAnswer.add(ab);
                solve = false;
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
